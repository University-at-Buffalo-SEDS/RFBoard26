...
//
// UPDATED:
// - Maintains a globally-accessible GPS-synced offset (ms) that other threads (telemetry thread)
...
//
// Notes:
// - This is "time-of-day" sync (seconds since midnight UTC), because your current GPS time packet
//   appears to be "seconds since midnight". If you later expose full date (Y/M/D) or Unix epoch,
//   we can make this a true epoch offset.
//
...
//       uint64_t gps_tod_ms = (local_ms % 86400000ULL) + (int64_t)g_gps_time_offset_ms;
//   }
...
static uint64_t tx_now_ms(void) {
    ...
    // (Still fine for offset sync; if you want better precision, accumulate remainder.)
...
static void gps_offset_update_ms(int64_t measured_offset_ms) {
    ...
    if (step == 0) {
        if (measured_offset_ms > 0) step = 1;
        else if (measured_offset_ms < 0) step = -1;
    }
...
=======
>>>>>>> refs/remotes/origin/neom9n_driver
    ...
    // Your comment says "double seconds since midnight", but the variable type was uint64_t.
    // We'll treat the packed value as *milliseconds since midnight UTC* for best integer math.
...
=======
                ...
                // IMPORTANT: pack_time_data() must fill gps_time_of_day_ms as *ms since midnight UTC*
                // (or you should change it accordingly).
                pack_time_data(&gps_packet, &gps_time_of_day_ms);

                // Update global offset so other threads can compute GPS time-of-day from local ticks.
                // offset := gps_tod_ms - (local_tod_ms)
                ...
                // Handle wrap-around near midnight: choose the shortest signed delta in [-12h, +12h]
                const int64_t HALF_DAY = (int64_t)(86400000LL / 2LL);
                if (measured_offset > HALF_DAY) measured_offset -= (int64_t)86400000LL;
                else if (measured_offset < -HALF_DAY) measured_offset += (int64_t)86400000LL;
                ...
                // if ((dbg++ % 200) == 0) {
                //     char buf[96];
                //     int n = snprintf(buf, sizeof(buf), "GPS offset_ms=%lld", (long long)g_gps_time_offset_ms);
                //     if (n > 0) log_telemetry_asynchronous(SEDS_DT_MESSAGE_DATA, buf, 1, (size_t)n + 1);
                // }
            } else {
>>>>>>> refs/remotes/origin/neom9n_driver
            ...
            if (consecutive_errors >= 5) {
                ...
                switch (status) {
                    case NEOM9N_TIMEOUT:   error_type = "GPS ERROR: Timeout";   break;
                    case NEOM9N_SPI_ERR:   error_type = "GPS ERROR: SPI Error"; break;
                    case NEOM9N_PARSE_ERR: error_type = "GPS ERROR: Parse Error"; break;
                    default:               error_type = "GPS ERROR: Undefined"; break;
                }

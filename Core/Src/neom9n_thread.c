...
static uint64_t tx_now_ms(void) {
  ...
  return ((uint64_t)(uint32_t)ticks * 1000ULL) /
         (uint64_t)TX_TIMER_TICKS_PER_SECOND;
}

static uint64_t gps_time_of_day_ms_from_packet(const NEOM9N_t *p) {
  // If you want to be stricter, clamp/validate ranges here.
  // This matches "ms since midnight UTC".
  uint64_t h = (uint64_t)p->hours;
  uint64_t m = (uint64_t)p->minutes;
  uint64_t s = (uint64_t)p->seconds;
  uint64_t ms = (uint64_t)p->milliseconds;

  return (((h * 60ULL + m) * 60ULL + s) * 1000ULL) + ms;
}
...
void neom9n_thread_entry(ULONG initial_input) {
  ...
  log_telemetry_asynchronous(SEDS_DT_MESSAGE_DATA, started_txt, 1,
  ...
  log_telemetry_asynchronous(SEDS_DT_MESSAGE_DATA, init_txt, 1,
                             sizeof(init_txt));

  // Binary buffers for telemetry
  uint8_t gps_data_buffer[12]; // 3 floats (lat, lon, alt)

  // Your comment says "double seconds since midnight", but the variable type
  // was uint64_t. We'll treat the packed value as *milliseconds since midnight
  // UTC* for best integer math.
  ...
  for (;;) {
    NEOM9N_status_t status =
        receive_nmea(&gps_packet, NMEA_MAX_WAIT, NMEA_MAX_IGNORES);
    ...
    if (status == NEOM9N_OK) {
      ...
      if (gps_has_fix(&gps_packet)) {
        ...
        log_telemetry_asynchronous(SEDS_DT_GPS_DATA, gps_data_buffer, 3,
                                   sizeof(float));
        ...
        gps_time_of_day_ms = gps_time_of_day_ms_from_packet(&gps_packet);
        ...
        int64_t measured_offset =
            (int64_t)gps_time_of_day_ms - (int64_t)local_tod;
        ...
        if (++no_fix_counter >= 100) {
          ...
          log_telemetry_asynchronous(SEDS_DT_MESSAGE_DATA, no_fix_txt, 1,
                                     sizeof(no_fix_txt));
          no_fix_counter = 0;
        }
      }
      ...
      if (consecutive_errors >= 5) {
        ...
        log_telemetry_asynchronous(SEDS_DT_GENERIC_ERROR, error_type, 1,
                                   strlen(error_type) + 1);
...
void create_neom9n_thread(void) {
  UINT status =
      tx_thread_create(&neom9n_thread, "NEOM9N Thread", neom9n_thread_entry, 0,
                       neom9n_thread_stack, NEOM9N_THREAD_STACK_SIZE, 5, 5,

...
#define TELEMETRY_TIME_MASTER 1
  ...
  // Schedule a time-sync response to be sent from the telemetry thread (non-blocking)
  SedsResult telemetry_schedule_timesync_response(uint64_t seq, uint64_t t1, uint64_t t2, uint64_t t3);

  // Flush any scheduled responses; intended to be called from telemetry thread loop.
  void telemetry_timesync_process_queue();
